...
void Administrateur::deleteById(int id) {
    for (auto it = ressources.begin(); it != ressources.end(); ++it) {
        if ((*it)->getId() == id) {
            ...
            ressources.erase(it);  // Suppression du vecteur
            std::cout << "[Admin] Ressource avec ID " << id << " supprimée." << std::endl;
            return;
        }
    }

    std::cout << "[Admin] Aucune ressource trouvée avec l'ID " << id << std::endl;
}
...
void Administrateur::reset() {
    ...
    std::cout << "[Admin] Toutes les ressources ont été supprimées." << std::endl;
}
